...
/*   Updated: 2022/09/13 16:11:40 by hnaciri-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
struct client_info {
    ...
    int socket;
    char request[BSIZE + 1];  // all of data receivd from the client stored here
...
void serve_resource(struct client_info **client_list, struct client_info *client, const char *path) {
    ...
    if (strcmp(path, "/") == 0) path = "/413.html";
    ...
    char full_path[128];
    sprintf(full_path, "error_pages%s", path);
    ...
    while(1) {
        ...
        while(client) {
            ...
            if (FD_ISSET(client->socket, &reads)) {
                ...
                if (BSIZE == client->received) {
                    send_400(&client_list, client);
                    client = next;
                    continue;
                }

                int r = recv(client->socket,client->request + client->received,BSIZE - client->received, 0);

               std::cout << client->request << " | " << std::endl << r << std::endl;
